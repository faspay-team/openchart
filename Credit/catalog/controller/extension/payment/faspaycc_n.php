...
class ControllerExtensionPaymentFaspayccN extends Controller {
	public function index() {
		...
		$setting = $this->model_setting_setting->getSetting('faspaycc_n', 0);

		$data['button_confirm'] 	= $this->language->get('button_confirm');
		$data['button_back'] 		= $this->language->get('button_back');

		$data['text_title']		  	= $this->language->get('text_title');
		$data['text_instruction'] 	= $setting['faspaycc_n_title'];
		...
		$data['mid']				= $setting['faspaycc_n_merchant_id'];
		$data['pas']				= $setting['faspaycc_n_merchant_pwd'];
		$data['merchant']			= $setting['faspaycc_n_title'];

		$data['order_id'] 			= $this->session->data['order_id'];
		$data['min_order']			= $setting['faspaycc_n_minimum_price'];
