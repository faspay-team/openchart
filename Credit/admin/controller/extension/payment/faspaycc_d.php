<?php
class ControllerExtensionPaymentFaspayccD extends Controller{
	private $error = array();

	public function index(){

		$this->load->language('extension/payment/faspaycc');
		$this->load->language('extension/payment/faspaycc_d');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			$this->load->model('setting/setting');
			$this->model_setting_setting->editSetting('faspaycc_d', $this->request->post);
        ...
        $data[$language_entry] = $this->language->get($language_entry);
      	...
      	$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/payment/faspaycc_d', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);

   		$data['action'] = $this->url->link('extension/payment/faspaycc_d', 'token=' . $this->session->data['token'], 'SSL');

    	$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'], 'SSL');

    	$inputs = array(
    		'faspaycc_d_geo_zone_id',
    		'faspaycc_d_sort_order',
    		'faspaycc_d_status',
    		'faspaycc_d_title',
    		'faspaycc_d_merchant_id',
    		'faspaycc_d_merchant_pwd',
    		'faspaycc_d_minimum_price'
    	);
    ...
    $this->template = 'extension/payment/faspaycc_d.tpl';
