<?php
class ControllerExtensionPaymentFaspayccE extends Controller{
	private $error = array();

	public function index(){

		$this->load->language('extension/payment/faspaycc');
		$this->load->language('extension/payment/faspaycc_e');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			$this->load->model('setting/setting');
			$this->model_setting_setting->editSetting('faspaycc_e', $this->request->post);
        ...
        $data[$language_entry] = $this->language->get($language_entry);
      	...
      	$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/payment/faspaycc_e', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);

   		$data['action'] = $this->url->link('extension/payment/faspaycc_e', 'token=' . $this->session->data['token'], 'SSL');

    	$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'], 'SSL');

    	$inputs = array(
    		'faspaycc_e_geo_zone_id',
    		'faspaycc_e_sort_order',
    		'faspaycc_e_status',
    		'faspaycc_e_title',
    		'faspaycc_e_merchant_id',
    		'faspaycc_e_merchant_pwd',
    		'faspaycc_e_minimum_price'
    	);
      ...
      $this->load->model('localisation/order_status');
    	...
    	$this->template = 'extension/payment/faspaycc_e.tpl';
