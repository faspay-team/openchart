<?php
class ControllerExtensionPaymentFaspayccB extends Controller{
  private $error = array();

  public function index(){

    $this->load->language('extension/payment/faspaycc');
    $this->load->language('extension/payment/faspaycc_b');
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
      $this->load->model('setting/setting');
      $this->model_setting_setting->editSetting('faspaycc_b', $this->request->post);
    ...
    foreach ($language_entries as $language_entry) {
      ...
      'href'      => $this->url->link('extension/payment/faspaycc_b', 'token=' . $this->session->data['token'], 'SSL'),
          'separator' => ' :: '
      );

      $data['action'] = $this->url->link('extension/payment/faspaycc_b', 'token=' . $this->session->data['token'], 'SSL');

      $data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'], 'SSL');

      $inputs = array(
        'faspaycc_b_geo_zone_id',
        'faspaycc_b_sort_order',
        'faspaycc_b_status',
        'faspaycc_b_title',
        'faspaycc_b_merchant_id',
        'faspaycc_b_merchant_pwd',
        'faspaycc_b_minimum_price'
      );
    ...
    $this->template = 'extension/payment/faspaycc_b.tpl';
