<?php
class ControllerExtensionPaymentFaspayccK extends Controller{
	private $error = array();

	public function index(){

		$this->load->language('extension/payment/faspaycc');
		$this->load->language('extension/payment/faspaycc_k');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			$this->load->model('setting/setting');
			$this->model_setting_setting->editSetting('faspaycc_k', $this->request->post);
        ...
        $data[$language_entry] = $this->language->get($language_entry);
      	...
      	$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/payment/faspaycc_k', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);

   		$data['action'] = $this->url->link('extension/payment/faspaycc_k', 'token=' . $this->session->data['token'], 'SSL');

    	$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'], 'SSL');

    	$inputs = array(
    		'faspaycc_k_geo_zone_id',
    		'faspaycc_k_sort_order',
    		'faspaycc_k_status',
    		'faspaycc_k_title',
    		'faspaycc_k_merchant_id',
    		'faspaycc_k_merchant_pwd',
    		'faspaycc_k_minimum_price'
    	);
      ...
      $this->load->model('localisation/order_status');
    	...
    	$this->template = 'extension/payment/faspaycc_k.tpl';
