...
class ControllerExtensionPaymentFaspayccO extends Controller{
	...
	public function index(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			$this->load->model('setting/setting');
			$this->model_setting_setting->editSetting('faspaycc_o', $this->request->post);
        ...
        $data[$language_entry] = $this->language->get($language_entry);
      	...
      	$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/payment/faspaycc_o', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);

   		$data['action'] = $this->url->link('extension/payment/faspaycc_o', 'token=' . $this->session->data['token'], 'SSL');

    	$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'], 'SSL');

    	$inputs = array(
    		'faspaycc_o_geo_zone_id',
    		'faspaycc_o_sort_order',
    		'faspaycc_o_status',
    		'faspaycc_o_title',
    		'faspaycc_o_merchant_id',
    		'faspaycc_o_merchant_pwd',
    		'faspaycc_o_minimum_price'
    	);
      ...
      $this->load->model('localisation/order_status');
    	...
    	$this->template = 'extension/payment/faspaycc_o.tpl';
