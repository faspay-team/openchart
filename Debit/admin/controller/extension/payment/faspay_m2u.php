<?php
class ControllerExtensionPaymentFaspayM2u extends Controller {
	...
	public function index() {
		...
		$this->load->language('extension/payment/faspay_m2u');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			$this->load->model('setting/setting');
			$this->model_setting_setting->editSetting('faspay_m2u', $this->request->post);
		...
		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/payment/faspay_m2u', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$data['action'] = $this->url->link('extension/payment/faspay_m2u', 'token=' . $this->session->data['token'], 'SSL');

    	$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'], 'SSL');

    	$inputs = array(
      		'faspay_m2u_server',
      		'faspay_m2u_geo_zone_id',
      		'faspay_m2u_expired_order',
      		'faspay_m2u_sort_order',
      		'faspay_m2u_status'
    	);
		...
		$this->template = 'extension/payment/faspay_m2u.tpl';
